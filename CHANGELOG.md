...
## Unreleased
### Added
  * Multiple cool badges


## [0.6.1] - 2026-08-26
### Added
  * Support for Python 3.10 and 3.14
  * `bump-my-version` integration with `make release-*` targets

### Changed
  * Switch to `uv`
  * `nox` is now a source of most commands, Makefile is just a thin wrapper around it
...
### Added
  ...
  * `PaymentStatus.ZERO` (`paymentStatus` = `0`). The API sends it although the specification does not define it, so any response carrying it was rejected with an `APIInvalidResponseError`
...
### Fixed
  ...
  * Falsy `paymentStatus` dropped from the response signature. The responses treated a `paymentStatus` of `0` as absent, so it was left out of the text rebuilt for the verification and the response was rejected with an invalid signature. Presence is now decided by the value being `null`, not by it being falsy
